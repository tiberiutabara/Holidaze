    ...
    <h3>Delete a hotel</h3>
        ...
        <select onChange={(e) => setHotelID(e.target.value)}>
          {hotel && hotel.map(hotel => 
            <option key={hotel.id} value={hotel.id}>{hotel.attributes.Title}</option>
        )}
        </select>
        <button onClick={() => {window.confirm('Are you sure that you want to delete this hotel?') === true ? deleteHotel() : alert('Delete process cancelled.')}}>Delete Hotel</button>
    </div>
  )
}
